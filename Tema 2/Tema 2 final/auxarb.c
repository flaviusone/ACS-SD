...
/*--- auxarb.c --- deseneaza arbore */
#include "Tema2.h"

static char desen[18][81], cri[] = "xxxxxxxxxxx";  /* zona reprezentare arbore, info */
...
{ int k;
	...
	sprintf(cri,"%3s", r->info);   /* converteste valoare info la 3 caractere */
...
void AfiArb(TArb r,FILE* f) /*- afiseaza arborele r -*/
{ int j;
	for (j = 0; j <= limita; j++) memset (desen[j], ' ', 79);
	if (!r) printf ("%49s", "-=- Arbore VID -=-");
	else
	{ Repr(r,40,0,19); /* pregateste reprezentare cu centrul in coloana 39 a primei linii */
		for (j = 0; j <= limita && desen[j][0] == ':'; j++)
		{ desen[j][79] = '\0'; fprintf(f,"%s\n", desen[j]+1); }
	}
	printf ("\n");
}
void AfiArb2(TArb r) /*- afiseaza arborele r -*/
{ int j;
	for (j = 0; j <= limita; j++) memset (desen[j], ' ', 79);
	if (!r) printf ("%49s", "-=- Arbore VID -=-");
	else
